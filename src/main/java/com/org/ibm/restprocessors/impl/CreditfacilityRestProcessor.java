...
@Service
public class CreditfacilityRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CreditfacilityRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.creditfacility.hosturl}")
	private String url;
	
	@Value("${spring.api.creditfacility.context-path}")
	private String contextPath;

	@Value("${spring.api.creditfacility.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CreditfacilityRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CreditfacilityRestProcessor.class.getName());
