...
@Service
public class ServiceorderRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ServiceorderRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.serviceorder.hosturl}")
	private String url;
	
	@Value("${spring.api.serviceorder.context-path}")
	private String contextPath;

	@Value("${spring.api.serviceorder.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ServiceorderRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ServiceorderRestProcessor.class.getName());
