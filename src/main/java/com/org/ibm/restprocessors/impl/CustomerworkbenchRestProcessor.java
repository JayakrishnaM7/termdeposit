...
@Service
public class CustomerworkbenchRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerworkbenchRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customerworkbench.hosturl}")
	private String url;
	
	@Value("${spring.api.customerworkbench.context-path}")
	private String contextPath;

	@Value("${spring.api.customerworkbench.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerworkbenchRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerworkbenchRestProcessor.class.getName());
