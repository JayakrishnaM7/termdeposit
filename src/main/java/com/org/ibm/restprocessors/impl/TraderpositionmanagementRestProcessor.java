...
@Service
public class TraderpositionmanagementRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(TraderpositionmanagementRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.traderpositionmanagement.hosturl}")
	private String url;
	
	@Value("${spring.api.traderpositionmanagement.context-path}")
	private String contextPath;

	@Value("${spring.api.traderpositionmanagement.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  TraderpositionmanagementRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ TraderpositionmanagementRestProcessor.class.getName());
