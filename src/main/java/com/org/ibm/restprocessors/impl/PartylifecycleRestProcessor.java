...
@Service
public class PartylifecycleRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PartylifecycleRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.partylifecycle.hosturl}")
	private String url;
	
	@Value("${spring.api.partylifecycle.context-path}")
	private String contextPath;

	@Value("${spring.api.partylifecycle.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PartylifecycleRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PartylifecycleRestProcessor.class.getName());
