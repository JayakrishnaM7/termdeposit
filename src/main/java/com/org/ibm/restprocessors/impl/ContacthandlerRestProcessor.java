...
@Service
public class ContacthandlerRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(ContacthandlerRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.contacthandler.hosturl}")
	private String url;
	
	@Value("${spring.api.contacthandler.context-path}")
	private String contextPath;

	@Value("${spring.api.contacthandler.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  ContacthandlerRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ ContacthandlerRestProcessor.class.getName());
