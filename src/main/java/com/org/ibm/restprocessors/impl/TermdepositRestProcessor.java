...
@Service
public class TermdepositRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(TermdepositRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.termdeposit.hosturl}")
	private String url;
	
	@Value("${spring.api.termdeposit.context-path}")
	private String contextPath;

	@Value("${spring.api.termdeposit.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  TermdepositRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ TermdepositRestProcessor.class.getName());
