...
@Service
public class CorporatetreasuryRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CorporatetreasuryRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.corporatetreasury.hosturl}")
	private String url;
	
	@Value("${spring.api.corporatetreasury.context-path}")
	private String contextPath;

	@Value("${spring.api.corporatetreasury.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CorporatetreasuryRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CorporatetreasuryRestProcessor.class.getName());
