...
@Service
public class CustomerproductandserviceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CustomerproductandserviceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customerproductandservice.hosturl}")
	private String url;
	
	@Value("${spring.api.customerproductandservice.context-path}")
	private String contextPath;

	@Value("${spring.api.customerproductandservice.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CustomerproductandserviceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CustomerproductandserviceRestProcessor.class.getName());
