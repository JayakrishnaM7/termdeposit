...
@Component
public class Termdeposit implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private TermdepositRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Termdeposit.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Termdeposit.class.getName());
         }else{
