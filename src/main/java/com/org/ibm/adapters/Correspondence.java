...
@Component
public class Correspondence implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private CorrespondenceRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Correspondence.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Correspondence.class.getName());
         }else{
