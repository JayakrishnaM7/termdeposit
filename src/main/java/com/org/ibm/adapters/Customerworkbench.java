...
@Component
public class Customerworkbench implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private CustomerworkbenchRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Customerworkbench.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Customerworkbench.class.getName());
         }else{
