...
@Component
public class Corporatetreasury implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private CorporatetreasuryRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Corporatetreasury.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Corporatetreasury.class.getName());
         }else{
